...
// EP-1 START
// ***
func (m *txSortedMap) flatten_matched(src string) types.Transactions {
	// If the sorting was not cached yet, create and cache it
	if m.cache == nil {
		m.cache = make(types.Transactions, 0, len(m.items))
		for _, tx := range m.items {
...
// EP-1 START
// ***
func (m *txSortedMap) FlattenMatched(src string) types.Transactions {
	// Copy the cache to prevent accidental modifications
...
func (l *txList) MatchedElement(src string) types.Transactions {
